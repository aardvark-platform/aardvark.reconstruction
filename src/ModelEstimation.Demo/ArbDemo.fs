...
module ArbDemo = 
    ...
    let showArbScenario 
        ...
        let statusText =
            ...
            Sg.text FontSquirrel.Hack.Regular C4b.White t
            |> Sg.scale 0.1
                ...
                let (name,recover,scene) =
                    ...
                    | HomographyScenario scene -> 
                        ...
                        let recover() =
                            ...
                            let c1 = scene.cam1
                            let scale = (c1.Location - c0.Location).Length
                            let prob = Homography.Ransac.problem c0.proj c1.proj 0.0025 
                            ...
                            if sol |> Seq.isEmpty then Log.warn "Ransac resulted in no homography"; []
                            else
                                let s = sol |> Seq.head
                                let hs = s.value
                                Log.line "Ransac succeeded with %d motions" hs.Length
                                hs |> List.map (fun m -> m.motion * scale)
                        name,recover,scene
