...
module Test = 
    ...
    let runManyExamples() =
        let tryAndAssert i =
            let c0, obs0, c1, obs1, p3d = Generate.randomScene()
            let scale = (c1.Location - c0.Location).Length
            let fMatches = 
                List.fold2 (fun r (_,os0) (_,os1) -> (List.zip os0 os1)@r ) [] obs0 obs1
            let hMatches =
                List.zip
                    (obs0 |> List.head |> snd)
                    (obs1 |> List.head |> snd)
            let obs3d = 
                List.zip obs1 p3d
                |> List.collect ( fun ((_,fs),(_,ps)) -> List.zip fs ps)
                |> List.toArray
            let hom = Homography.recover (hMatches |> List.toArray)
            let hmot =
                match hom with
                | None -> 
                    Log.error "Recover H failed"
                    [CameraMotion.Zero]
                | Some h -> 
                    match Homography.decompose h c0.proj c1.proj [] with
                    | [] -> 
                        Log.error "Homography decompose failed"
                        [CameraMotion.Zero]
                    | hs -> 
                        hs |> List.collect (fun m -> [m * scale; m * -scale])
            let fund = FundamentalMatrix.recover 1E-6 (fMatches |> List.toArray)
            let fmot =
                match fund with
                | None -> 
                    Log.error "Recover F failed"
                    [CameraMotion.Zero]
                | Some (F,lsbr,rsbl) -> 
                    match FundamentalMatrix.decompose F c0.proj c1.proj [] with
                    | [] -> 
                        Log.error "Fundamental decompose failed"
                        [CameraMotion.Zero]
                    | fs -> 
                        fs |> List.collect (fun m -> [ m * scale; m * -scale ])
            let getBestFittingMot (mots : list<CameraMotion>) =
                mots |> List.maxBy (fun mot -> 
                    let r = c0 + mot
                    let dir = abs (Vec.dot c1.Forward r.Forward) * 1000.0  |> int
                    let pos = -Vec.length (c1.Location - r.Location)
                    (dir, pos)
                )
            let cp =
                match P6P.recover obs3d with
                | None -> 
                    Log.error "P6P failed"
                    c0
                | Some cp -> 
                    { cp with proj = { cp.proj with distortion = { cp.proj.distortion with imageSize = c1.proj.distortion.imageSize } } }
            ...
            if 
             ...
             not ( Camera.approxEqual 1E-4 c1 cf ) then
                ...
                Pickler.pickler.Pickle((c0, obs0, c1, obs1, p3d)) |> File.writeAllBytes @"D:\bla\dump.bin"
    ...
    let createAndRenderExampleTest() =
        Ag.initialize()
        Aardvark.Init()
        let win = window {
            backend Backend.GL
        ...
        let stuff =
            counter |> Mod.map ( fun _ -> 
                ...
                let c0, obs0, c1, obs1, p3d = Generate.randomScene()
                //let c0, obs0, c1, obs1, p3d : Camera*((V3d*V3d)*V2d list) list*Camera*((V3d*V3d)*V2d list) list*((V3d*V3d)*V3d list) list = File.readAllBytes @"D:\bla\dump.bin" |> Pickler.pickler.UnPickle
                let scale = (c1.Location - c0.Location).Length
                Log.stop()
                ...
                let fmot =
                    ...
                    | Some (F,lsbr,rsbl) -> 
                        ...
                        let realF = FundamentalMatrix.fromCameras c0 c1
                        match FundamentalMatrix.decompose F c0.proj c1.proj [] with
                        | [] -> 
                            Log.warn "Fundamental decompose failed"
                            [CameraMotion.Zero]
                        | fs -> 
                            let realM = FundamentalMatrix.decompose realF c0.proj c1.proj []

                            let m0 = realM |> List.map (fun m -> m * scale)
                            let m1 = fs    |> List.map (fun m -> m * scale)

                            let allGood = m0 |> List.map (fun m0 -> m1 |> List.exists (fun m1 -> CameraMotion.similarity m0 m1 > 1.0 - 1E-4)) |> List.fold (&&) true
                            let anyGood = m0 |> List.map (fun m0 -> m1 |> List.exists (fun m1 -> CameraMotion.similarity m0 m1 > 1.0 - 1E-4)) |> List.fold (||) false

                            if not allGood then Log.warn "aaaaaaaa"
                            if not anyGood then Log.error "bbbbbbb"
                ...
                Log.stop()
                Generate.mkSg p3d,c0,c1,cp,ch,cf,obs0,obs1
            )
    
        let sg = Mod.map   (fun (x,_,_,_,_,_,_,_) -> x) stuff
        let c0 = Mod.map   (fun (_,x,_,_,_,_,_,_) -> x) stuff
        let c1 = Mod.map   (fun (_,_,x,_,_,_,_,_) -> x) stuff
        let cp = Mod.map   (fun (_,_,_,x,_,_,_,_) -> x) stuff
        let ch = Mod.map   (fun (_,_,_,_,x,_,_,_) -> x) stuff
        let cf = Mod.map   (fun (_,_,_,_,_,x,_,_) -> x) stuff
        let obs0 = Mod.map (fun (_,_,_,_,_,_,x,_) -> x) stuff
        let obs1 = Mod.map (fun (_,_,_,_,_,_,_,x) -> x) stuff
        ...
        let ftrSg = 
            Mod.custom (fun t -> 
                ...
                let obs0 = obs0.GetValue(t)
                let obs1 = obs1.GetValue(t)
                let obs = 
                    match current with 
                    | 1 -> obs0
                    | 2 | 4 | 5 -> obs1 
