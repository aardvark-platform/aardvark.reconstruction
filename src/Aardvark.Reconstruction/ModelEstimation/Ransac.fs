...
// Fast parallelized Ransac implementation with early exit strategy.
...
module RansacProblem =
    ...
    let inline map (fTrain : 'd -> 'e) (fTrainInv : 'e -> 'd) (fTest : 't -> 'u) (fTestInv : 'u -> 't) (fVal : 's -> 'r) (prob : IRansacProblem<'d,'t,'s>) : IRansacProblem<'e,'u,'r> = 
        { new IRansacProblem<'e,'u,'r> with
            member _.Solve(cfg,train,test) = 
                prob.Solve(cfg, train |> Array.map fTrainInv, test |> Array.map fTestInv) 
                    |> Seq.map (fun r -> 
                        {   data            = r.data |> Array.map fTrain
                            test            = r.test |> Array.map fTest 
                            value           = r.value |> fVal
